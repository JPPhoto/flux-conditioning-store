    ...
    def invoke(self, context: InvocationContext) -> FluxConditioningOutput:
        ...
        for i, cond_field in enumerate(all_input_conditionings):
            ...
            if cond_field is not None:
                ...
                try:
                    conditioning_data: ConditioningFieldData = context.conditioning.load(cond_field.conditioning_name)
        ...
        for original_input_num, flux_info, strength in connected_inputs:
            if flux_info.t5_embeds is None:
                warning(
                    f"T5 embeddings missing for conditioning from input {original_input_num}. Skipping for T5 concatenation."
                )
            else:
            ...
            if flux_info.clip_embeds is None:
                warning(
                    f"CLIP embeddings missing for conditioning from input {original_input_num}. Skipping for CLIP processing."
                )
            else:
        ...
        if self.select_clip == 0:
            ...
            if selected_clip_tensor is None:
                info(
                    f"CLIP embedding not found directly for input {requested_input_num}. Searching for next subsequent available."
                )
                for original_num, clip_tensor in clip_tensors_with_original_indices:
                    if original_num > requested_input_num:
                        selected_clip_tensor = clip_tensor
            ...
            if selected_clip_tensor is None:
                info("No subsequent CLIP embedding found. Searching for preceding available.")
            ...
            if selected_clip_tensor is None:
                ...
                error(
                    f"Could not find a suitable CLIP embedding for selected input {requested_input_num} despite available inputs. This indicates an unexpected state in selection logic."
                )
            ...
            error(
                f"Invalid select_clip value: {self.select_clip}. Must be 0 (zeros) or a positive integer (1-indexed input selection)."
            )
        ...
        if final_clip_embeds is None:
            error(
                "Failed to determine final CLIP embeddings. This should not happen after the selection logic has been applied."
            )
        ...
        for i, t5_embeds in enumerate(all_t5_embeds_tensors[1:]):  # Start from the second item if more exist
            ...
            info("Concatenating T5 embeddings...")
