    ...
    def invoke(self, context: InvocationContext) -> RetrieveFluxConditioningMultiOutput:
        ...
        if isinstance(self.conditioning_id_or_list, str):
            ...
            return RetrieveFluxConditioningMultiOutput(
                conditioning=FluxConditioningField(conditioning_name=""), conditioning_list=[]
            )
        ...
        try:
            ...
            for cond_id in conditioning_ids_to_retrieve:  # Iterate through the sorted input IDs
                if cond_id in results_map:
                    ...
                    info(
                        f"Retrieved and re-saved conditioning with ID: {cond_id} as new conditioning name: {conditioning_name}"
                    )
        ...
        if retrieved_flux_conditioning_fields:
            ...
            info(
                f"Selected conditioning at index {self.select_index} (effective index {effective_index}) for single output."
            )
        else:
        ...
        return RetrieveFluxConditioningMultiOutput(
            conditioning=selected_conditioning_output, conditioning_list=retrieved_flux_conditioning_fields
        )
